...
/* Simple line chart */
const LineChart = ({ data, width = 400, height = 160, color = ChartColors.blue, yLabel = '', threshold, thresholdLabel }) => {
...
/* Bar chart */
const BarChart = ({ data, width = 400, height = 160, color = ChartColors.blue, yLabel = '' }) => {
  if (!data || data.length === 0) return null;
  const pad = { top: 20, right: 16, bottom: 28, left: 44 };
  const w = width - pad.left - pad.right;
  const h = height - pad.top - pad.bottom;
...
/* Wind rose (simplified 8-direction) */
const WindRose = ({ data, size = 180 }) => {
...
/* Mini sparkline */
const Sparkline = ({ values, width = 80, height = 24, color = ChartColors.blue }) => {
...
/* Gauge meter */
const GaugeMeter = ({ value, max, threshold, label, unit, size = 120 }) => {
  const r = (size - 20) / 2;
  ...
  const color = value > (threshold || max) ? ChartColors.red : value > (threshold || max) * 0.8 ? ChartColors.amber : ChartColors.blue;
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: size, height: size }}>
      <path d={`M ${arc(startAngle).x} ${arc(startAngle).y} A ${r} ${r} 0 1 1 ${arc(endAngle).x} ${arc(endAngle).y}`}
