/* ============================================
   Dashboard Page — Map + Overview Cards
   ============================================ */

const MapView = ({ sites, onSiteClick, selectedSite }) => {
  ...
  React.useEffect(() => {
    ...
    const statusColor = { ok: '#1a8a4a', warn: '#c27a0e', danger: '#c0392b' };
  ...
  return <div ref={mapRef} style={{ height: '100%', borderRadius: 'var(--radius-md)' }}></div>;
};

const SiteStatusCard = ({ site, onClick, density }) => {
  const w = generateWeather(site.id);
  const m = generateMarine(site.id);
            ...
            <div>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, fontVariantNumeric: 'tabular-nums',
                color: m.waveHeight > site.thresholds.waveHeight ? 'var(--status-danger)' :
                  m.waveHeight > site.thresholds.waveHeight * 0.8 ? 'var(--status-warn)' : 'inherit'
              }}>{m.waveHeight}m</div>
            </div>
          ) : (
            <div>
...
const DashboardPage = ({ navigate, density }) => {
    ...
    <div>
      ...
      <div className="grid-4 mb-16">
        <div className="stat-card">
          ...
          <div className="stat-sub">陸上 {SITES.filter(s=>s.type==='land').length} / 海上 {SITES.filter(s=>s.type==='marine'||s.type==='both').length}</div>
        </div>
        <div className="stat-card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${FORECAST_DAYS.length}, 1fr)`, gap: 8, textAlign: 'center' }}>
            {FORECAST_DAYS.map((d, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4, color: i === 0 ? 'var(--blue-500)' : 'var(--text)' }}>
                  {d.date}
                </div>
...
Object.assign(window, { DashboardPage, MapView, SiteStatusCard });
