...
最終更新: **2026-05-27**（Loop 20 — `dashboard.jsx → src/dashboard.tsx` ESM port 着地）
...
| 🖥️ Frontend | 🟡 97% | `4d04354` | Vite scaffold ✅／Phase 1 `api.ts` + `charts.tsx` + `data.ts` + `decisions.tsx` + `dashboard.tsx` ESM 化 ✅／残: app.jsx (root) の ESM 移植・残 12 Page コンポーネント・E2E |
