...
最終更新: **2026-05-27**（Loop 21 — `weather-marine.jsx → src/weather-marine.tsx` ESM port 着地）
...
| 🖥️ Frontend | 🟡 98% | `1f4c915` | Vite scaffold ✅／Phase 1 `api.ts` + `charts.tsx` + `data.ts` + `decisions.tsx` + `dashboard.tsx` + `weather-marine.tsx` ESM 化 ✅／残: app.jsx (root) の ESM 移植・残 10 Page コンポーネント・E2E |
