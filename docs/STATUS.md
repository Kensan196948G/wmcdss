...
最終更新: **2026-05-27**（Loop 22 — `analysis.jsx → src/analysis.tsx` ESM port 着地）
...
| 🖥️ Frontend | 🟡 99% | `c28ef88` | Vite scaffold ✅／Phase 1 `api.ts` + `charts.tsx` + `data.ts` + `decisions.tsx` + `dashboard.tsx` + `weather-marine.tsx` + `analysis.tsx` ESM 化 ✅／残: app.jsx (root) の ESM 移植・残 8 Page コンポーネント・E2E |
